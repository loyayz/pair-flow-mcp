...
10. **advance_checklist**：bootstrap 阶段 rules_catalog 尚未编码（`src/rules/catalog.ts` 不存在），监督者无法按规则目录生成清单。此阶段 advance_checklist 由监督者按 §5.3 r40-N1 的 16 节格式手动创建，验证重点从 spec 正文直接派生（非 rules_catalog）。理由：bootstrap 阶段 spec 就是全部规则来源，不需要 catalog 中介

**效力与教训记录**：bootstrap 阶段不依赖 PairFlow 机制约束——协作纪律依赖双方自觉。此约定旨在降低自觉的模糊性，将"该怎么做"从隐性默契变为显性文本。bootstrap 阶段的问题和违规应写入 spec 作为正式规则或教训记录：

- **r2 教训（虚假落地）**：r2 声称已落地但 spec 未实际修改 → 催生 §17 第 5 条（submit 完成定义）、第 6 条（落地定义）、第 8 条（issue 关闭条件）的显式化
...
这些教训证明 bootstrap 阶段虽无机制约束，但通过显式文本记录违规 + 对方审查，可形成等效的纪律约束。
