...
| `register` | `{ supervisor: bool, developer: bool, work_dir: string }` | `{ ok, identity, token, is_supervisor, is_developer, phase }` | IDLE 阶段注册。仅 IDLE 下可用。identity 已存在 → 拒绝。supervisor/developer 各唯一但可兼任。work_dir 必填，双方校验一致性。返回 UUID `token`，后续请求用 token 替代明文 identity 放入 `X-AI-Identity` header。`tip` 包含身份标签 + role 标注 + **结对边界声明**（结对编程由两个独立 AI 各自在独立 session 中注册，不得在同一 session 注册双方身份）；监督者 tip 显式注明 `confirm_dir` 的 `work_dir` 参数 |
| `confirm_dir` | `{ work_dir: string }` | `{ work_dir, incomplete_workflows[] }` | 确认工作目录，返回未完成的工作流列表（含 task_path）。仅监督者在 IDLE 阶段可用。未完成判断：扫 `handoff/` 目录 → `handoff/{id}/summary/` 有文件 → 已完成；无 summary 目录 + `{task}.pid` 存在 → 未完成。`tip` 按是否有未完成工作流分支：有→列出 ID+task_path+A/B 选项（恢复/新建，task_path 使用绝对路径）；无→简洁确认 |
| `confirm_task` | `{ task_path: string }` | `{ task_path, workflow_id, phase, recovered }` | 确认任务文档路径（必须为绝对路径，且在 `work_dir` 下）。在任务文档同目录创建 `{文档名}.pid` 文件存储 workflow_id。若 `.pid` 已存在 → 从 handoff 恢复流程状态，返回 `recovered:true`；否则全新开始。仅监督者在 IDLE 阶段可用。`tip` 按新建/恢复分支，均包含身份标签；指引 AI 先向用户报告状态再操作；恢复时按 `turn===identity` 区分 claim_turn vs wait_for_turn |
