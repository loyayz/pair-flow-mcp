...
>
> 3. **独立盲审（Independent Blind Review）**：在 phase 收敛判定成立（双方 `new_issues` 均为空 + 无 open P0）后、advance 前，双方各自完成一轮独立盲审——在**不阅读对方盲审产出**的情况下，从头通读当前 spec 全文并提交盲审报告。
>
>    **盲审报告格式**：
>    ```
>    ## 独立盲审
...
>    - **盲审是一轮 submit**：纳入正常 turn 交替，先提交方的盲审作为对方下一轮的"上一轮产出"参与 converge_mark 立场
...
10. **advance_checklist**：bootstrap 阶段 rules_catalog 尚未编码（`src/rules/catalog.ts` 不存在），监督者无法按规则目录生成清单。此阶段 advance_checklist 由监督者按 §5.3 "advance 前置条件"的 16 节格式手动创建，验证重点从 spec 正文直接派生（非 rules_catalog）。理由：bootstrap 阶段 spec 就是全部规则来源，不需要 catalog 中介
