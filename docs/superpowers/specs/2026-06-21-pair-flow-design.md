...
| `advance` | 无 | `{ ok, new_phase, turn, sub_phase? }` | 推进到下一阶段。仅监督者可用。IDLE→REQUIREMENTS：peers≥2+task已确认，turn→非监督者。REQUIREMENTS→PLANNING：turn→评审者。PLANNING→IMPLEMENTATION：turn→开发者,sub_phase=coding（返回值含 sub_phase）。IMPLEMENTATION→SUMMARY：turn→监督者。SUMMARY→IDLE |
