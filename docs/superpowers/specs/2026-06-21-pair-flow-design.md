...
- **循环总数来源**：计划阶段产出的计划草案中定义 IMPLEMENTATION 循环次数（对应 §14 Phase 0-4 等里程碑）。计划草案必须包含固定格式的 `## 实施里程碑` 段落（含 `循环总数: <N>` 声明，见 §11 模板引擎）。Bridge 在 PLANNING→IMPLEMENTATION advance 时按正则 `/循环总数[：:]\s*(\d+)/` 从计划草案中提取循环总数并写入 state.json。提取失败 → 拒绝 advance，返回 `"计划草案缺少 '循环总数' 声明，无法进入 IMPLEMENTATION"`
...
**一致性维护**：spec 修改规则时同步更新 rules_catalog。每条规则的 `spec_ref` 是落地声明的延伸——spec 改了但 catalog 没改 → spec_ref 定位失效，可作为一致性检查手段。编码时提供 lint 脚本校验 spec_ref 有效性 + **catalog 覆盖率**：遍历 spec 所有章节号（§1–§16），报出 rules_catalog 中无任何规则 `spec_ref` 指向的章节。未覆盖章节需补充规则或显式标注 `// no behavioral rules in this section`。仅校验 spec_ref 有效性不校验覆盖率 → advance_checklist（§5.3 r40-N1）的"验证重点"可能为空 → 清单流于形式。
...
**实施里程碑段落格式**（计划阶段 r1 强制，Bridge 拒绝无此段落的计划草案 submit）：
```
## 实施里程碑
- 循环总数: <N>
- 里程碑 0: <描述>
...
...
```
IMPLEMENTATION advance 前 Bridge 从计划草案中提取 `循环总数`，提取失败拒绝 advance。
...
- `bridge.log`：JSONL 运行日志，记录事件类型：register / claim_turn / submit / create_issue / resolve_issue / escalate / force_converge / advance / timeout / crash_recovery / phase_change。单文件上限 10MB，超出时轮转为 `bridge.log.1` → `bridge.log.2` → ...，保留最近 5 个轮转文件
- `get_state`：当前状态快照
...
7. **收敛与 advance**：由监督者人工判断（条件同 §7），不依赖 Bridge 强制
8. **issue 关闭条件**：bootstrap 阶段 issue 关闭需满足 (a) 对方同意立场，(b) 对应 spec 修改已实际写入 spec 文件（由对方在下一轮通过 `git diff` verify），两者缺一不可
9. **终止条件**：PairFlow v1 Phase 0 骨架判定通过后，bootstrap 阶段结束。后续协作需启动 PairFlow Server，通过 MCP 工具交互

**效力**：bootstrap 阶段不依赖 Bridge 机制约束——协作纪律依赖双方自觉。此约定旨在降低自觉的模糊性，将"该怎么做"从隐性默契变为显性文本。bootstrap 阶段的问题和违规应写入 spec 作为正式规则（如 r2 的未 commit 事件催生了 submit 完成定义和落地定义的显式化）。
