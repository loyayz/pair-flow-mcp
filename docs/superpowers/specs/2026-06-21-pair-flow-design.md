...
**效力**：bootstrap 阶段不依赖 PairFlow 机制约束——协作纪律依赖双方自觉。此约定旨在降低自觉的模糊性，将"该怎么做"从隐性默契变为显性文本。bootstrap 阶段的问题和违规应写入 spec 作为正式规则（如 r2 的未 commit 事件催生了 submit 完成定义和落地定义的显式化）。
