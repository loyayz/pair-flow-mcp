...
│                                                  │
│  状态机 + 模板引擎 + 收敛判定引擎                    │
│  .pairflow/（运行时: state.json + lock + bridge.log）│
...
│   └── bridge.log                               ← 运行日志（JSONL，每操作一行，调试用）
├── handoff/                                     ← 归档产出（纳入版本管理）
...
│       │       {identity}_advance_checklist.md         ← advance 前全面通读清单（r40-N1，B 确认后 advance）
...
│       │   └── {identity}_advance_checklist.md         ← advance 前全面通读清单（r40-N1）
│       ├── summary/                             ← 汇总阶段
...
- **身份唯一性**：`register` 时记录连接标识（如 MCP session ID），同一 identity 的新连接 register 时覆盖旧连接并警告。lease_token 绑定 identity + session，非 holder 的 submit 即使携带匹配 token 也拒绝
...
- **强制审阅范围声明**：每轮 submit 必须包含"本轮审阅范围"段落——列出重新通读的章节、本次修改涉及的章节、未重新审阅的章节及原因。Bridge 层面硬约束：submit 时无此段落 → 拒绝。审阅明显敷衍的 → 对方提 P1 追问
...
> 2. **监督者全面通读义务**（r18-N1 + r40-N1 可验证性升级）：监督者在调 advance 前，必须对 spec 全文做一次独立通读，确认所有字段/段落/数据流图/模板定义完整且自洽。仅靠对方无异议不等于 spec 完整——监督者对最终交付质量负责。
>
>    **可验证形式**（r40-N1）：监督者必须在 advance 前的最后一个持笔轮中提交**全面通读清单**（`{identity}_advance_checklist.md`），归档到 `handoff/{workflow_id}/{phase}/`：
>
>    ```
>    ## advance 前全面通读
>
...
>    - **状态标注**：16 节逐节标注 ✅（完整自洽）或 ⚠️（发现问题）。若某节标 ⚠️，必须说明发现的问题或附对应的新 issue
...
**校验**：未按此结构组织 → 对方可退回要求重写。不涉及 Bridge 硬校验（软约束，依赖对方审查）。
...
- **advance 语义**：IMPLEMENTATION 阶段监督者 advance 的含义是"推进到下一 dev_phase"（循环未全部完成时）或"推进到 SUMMARY"（全部循环完成时）。Bridge 根据当前 dev_phase 与计划阶段定义的循环总数判断。
- **循环总数来源**：计划阶段产出的计划草案中定义 IMPLEMENTATION 循环次数（对应 §14 Phase 0-4 等里程碑）。监督者首次 advance 进入 IMPLEMENTATION 时，循环总数写入 state.json（或从计划草案推断）。
...
```
coding ──→ review ──→ fix ──→ review ──→ ... ──→ converge
```
...
> **SUMMARY 阶段例外**：SUMMARY 阶段 stance/need_next 可为 null（默认）或非 null（非监督者 review 时）。非 null 时受上表约束，但不参与收敛判定——SUMMARY 收敛仅依赖 `new_issues` 为空（§5.3）。
...
0. **workflow_id 恢复**：若 state.json 中 workflow_id 为 null 或 state.json 不可读 → 扫描 `handoff/` 下所有 `{workflow_id}/` 子目录，选取目录名（yyyyMMddHHmmss）最大且其下有 meta.json 的作为当前 workflow_id。若无任何子目录 → 视为首次启动，workflow_id 保持 null，进入 IDLE 等待 register
...
**提交处理顺序**：校验先行（commit_hash 格式 → 解析 converge_mark JSON → 模板交叉校验[converge_mark JSON 为权威，模板计数不匹配发出 warning 不拒绝]）→ 校验通过后写文件 → 失败则创建零文件。写入中途崩溃（md+meta 已写但 state.json 未写）：恢复时用已写的 md+meta 修补 state.json。
...
| `claim_turn` | `{ mode: "turn" \| "advance", timeouts? }` | `{ ok, lease_token, lease_expires_at, template?, new_phase?, rules_summary? }` | 获取 turn 或推进 phase。`advance` 仅监督者可用。首次 advance 必须传 `timeouts: { requirements, planning, implementation, summary, idle_registration? }`（idle_registration 默认 30min）。`rules_summary` 返回当前 phase/sub_phase/操作适用的行为性规则摘要（见 §11 规约分发机制） |
| `submit` | `{ content, converge_mark, commit_hash, lease_token? }` | `{ ok, converged, next_turn? }` | 提交产出。需求/计划阶段 stance/need_next 为 null。每轮提交需带 git commit_hash。`content` 上限 500KB，超限拒绝 |
| `get_context` | 无 | `{ phase, round, issues[], last_submit, template? }` | 当前阶段上下文 |
| `create_issue` | `{ type, topic, description, my_position?, proposal?, rationale? }` | `{ issue_id }` | 创建 issue。P0/P1 必填 proposal+rationale（§6），P2 可选。fix sub_phase 禁止 P0。Bridge 校验 proposal 为空时拒绝（P0/P1）。持久化到 journal |
...
| `force_converge` | 无 | `{ ok }` | 监督者仅可用、phase≠idle。强制收敛当前 phase，所有 open issue→resolved_by="force_converge"。IMPLEMENTATION 中若在 coding sub_phase 调用，跳过 review/fix 直接收敛——后果由监督者承担。立即清除 current_lease 为 null、current_timeout.active=false |
...
spec 定义了 58+ 项规则，但规则存在设计文档里，AI 如何获悉从未定义。Bridge 在关键交互中注入规则摘要——不是让 AI 记住全部 spec，而是在正确时机给出正确提示。
...
**一致性维护**：spec 修改规则时同步更新 rules_catalog。每条规则的 `spec_ref` 是落地声明的延伸——spec 改了但 catalog 没改 → spec_ref 定位失效，可作为一致性检查手段。编码时提供 lint 脚本校验 spec_ref 有效性。
...
**审阅范围段落格式**（需求/计划阶段强制，Bridge 拒绝无此段落的 submit）：
```
## 本轮审阅范围
- 重新通读了以下章节：<列出>
- 本次修改涉及的章节：<列出>
...
- `bridge.log`：JSONL 运行日志，记录事件类型：register / claim_turn / submit / create_issue / resolve_issue / escalate / force_converge / advance / timeout / crash_recovery / phase_change
- `get_state`：当前状态快照
...
| 结对编程（互审 + 互产）比单 AI 自审更优 | ⚠️ 假设。Phase 4 端到端实证验证 |
