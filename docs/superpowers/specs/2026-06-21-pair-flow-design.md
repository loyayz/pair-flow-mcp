...
状态为进程内存变量（重启后丢失），handoff/ 下的 meta.json + 产出文件是权威来源。崩溃恢复从 handoff/ 重建（见 §8）。归档文件有版本管理价值（handoff/，git 提供追溯+备份）。
...
| `register` | `{ identity: string, work_dir: string }` | IDLE 阶段注册身份。identity 从 body 取。不声明角色（角色移至 confirm_task）。返回 UUID `token`。入参校验失败返回 curl 格式参考。`tip` 详细列出 confirm_task 的 5 个入参（task_path/task_type/supervisor/developer/work_dir）及含义 |
| `confirm_task` | `{ task_path, task_type?, supervisor, developer, work_dir }` | 确认任务文档并声明角色。两个 AI 使用相同 task_path 自动成对（最多 2 个 token，校验角色唯一性和 work_dir 一致性）。扫 `handoff/` 恢复未完成工作流。绑定 token→workflowId。`tip` 分场景：创建→等待对方加入；加入→双方已就位；已有身份→重新加入。后续所有工具通过 token 路由到对应工作流 |
