...
**问题处置由对方修改**：任何一方提出的问题，必须双方都同意才能关闭，且由**对方**执行修改——提出者不修改自己提的问题。此约束对所有持笔者平等适用，不因监督者身份豁免——监督者持笔时提出的 issue，由非监督者在下一持笔轮执行 spec 修改。**正式阶段 PairFlow 强制校验**：submit 时若 `resolved_issue_ids` 包含 `raised_by = 当前持笔者` 的 issue → 拒绝（"issue #N was raised by you; the other party must land the spec change"）。bootstrap 阶段靠对方审查 + §17 教训记录约束。
...
>    ```
>
>    （行数为 bootstrap 阶段含 §17 共 17 节，生产阶段 §1-§16 共 16 节）
>
>    **随机抽查验证流程**：
>    1. 监督者填完 checklist 提交
>    2. PairFlow 从 17 节中随机抽取 3 节，通知非监督者
...
>
>    **抽查博弈约束**：监督者不知道哪 3 节会被查，理性策略是通读全部 17 节。若只读少量节碰运气，命中率 3/17≈18%, 失败概率高 + escalate 风险，通读是最优策略。
>
>    **Bootstrap 替代**：
>    1. 监督者产出 checklist（含 17 节验证重点+状态+验证重点来源）
>    2. 非监督者随机指定 3 节（§X, §Y, §Z），要求监督者补充"行号+行内容概述"——行号由监督者自选（须真实行号）
>    3. 监督者补充后，非监督者打开 spec 核对概述准确性
>    4. 3/3 通过 → checklist 确认；否则退回
>
...
>
> **Phase 间 issue defer 规则**：IMPLEMENTATION 阶段 dev_phase 循环收敛前，该循环发现的所有 issue 必须 (a) 已修复并关闭，或 (b) 标记为 `deferred` 并附**具体技术理由**说明为何当前 dev_phase 无法解决（如依赖后续 Phase 的实现）。监督者 advance 前检查 deferred 列表——无理由或理由不成立 → 拒绝 advance。连续 2 个 dev_phase defer 同一 issue → 自动升级为 P0，监督者与用户沟通裁定。此规则防止"defer to next phase"被滥用为逃避出口。
...
| `blind_review` | 双方各自 | `{identity}_blind_review.md` | 双方均提交后：无新 issue → advance_checklist；有新 issue → 回到 coding/fix 或交替持笔 |
...
**盲审子阶段**（各 phase 适用，IMPLEMENTATION 通过 sub_phase=blind_review 实现，需求/计划/SUMMARY 通过 turn 交替实现）：

- **执行者**：双方各自执行，非监督者先提交盲审，监督者后提交（turn 顺序见 §5.3 第 3 条）
- **推进条件**：双方盲审均 submit 后，PairFlow 检查双方 `new_issues`：
  - 均空 → `blind_review_pending=false`，进入 advance_checklist 流程
  - 任一方非空 → `blind_review_pending=false`，进入交替评审处理新 issue（IMPLEMENTATION 回到 coding/review/fix；需求/计划/SUMMARY 回到交替持笔），处理后重新收敛
- **盲审 submit 不触发收敛检查**——仅更新 `last_submit_per_turn`。收敛检查在双方盲审均提交后由 PairFlow 统一执行
- **非 IMPLEMENTATION 的盲审**：需求/计划/SUMMARY 阶段不使用 sub_phase 机制，盲审通过 turn 交替实现——收敛后 turn 切为非监督者，非监督者提交盲审 → turn 切为监督者，监督者提交盲审 → PairFlow 检查双方 new_issues
...
4a. 盲审文件（`{identity}_blind_review.md`）：恢复规则同普通 submit——`.md` + `.meta.json` 均存在且可解析 → 重建 history（类型标记为 `"blind_review"`，不推进 round，推进 turn）；仅 `.md` 无 `.meta` → 视为不完整，忽略。盲审文件的 `converge_mark` 中 `stance=null, need_next_round=null`。`blind_review_pending` 崩溃推断：若双方盲审 meta.json 均存在 → `false`；仅一方存在 → `true`，turn 保持在未提交方。单方盲审提交后崩溃（另一方未提交）：恢复后 `blind_review_pending=true`，turn 指向未提交方，等待其提交或超时
...
| `get_state` | 无 | `{ phase, sub_phase, dev_phase?, round, turn, converged, pending_supervisor_review, peers, issues[], escalation_recommended?, ... }` | 完整状态快照。匿名可用。`dev_phase` 仅 IMPLEMENTATION 阶段非 null（当前开发里程碑序号）；`pending_supervisor_review` 标识监督者最终 review 暂停状态；`blind_review_pending` 标识双方需完成盲审；`escalation_recommended` 在 P0 僵持检测触发时返回 issue ID 列表 |
...
| `submit` | `{ content, converge_mark, commit_hash, lease_token?, blind_review? }` | `{ ok, converged, next_turn? }` | 提交产出。需求/计划阶段 stance/need_next 为 null。每轮提交需带 git commit_hash（= 本轮 submit 所基于的仓库 HEAD，即 submit 前 `git rev-parse HEAD` 的结果，非本轮产出产生的 commit）。`content` 上限 500KB（经验值，Phase 4 验证后调整），超限拒绝并提示"请拆分产出或引用外部文件"。`blind_review: true` 标记盲审提交——盲审模式下：(1) `get_archived_file_content` 和 `get_archived_files` 对对方盲审文件返回拒绝（保证独立性）；(2) converge_mark 中 stance 和 need_next_round 必须为 null（盲审是发现导向，非立场表态），new_issues 可含 P0/P1/P2；(3) 盲审 submit 不触发收敛检查——仅更新 `last_submit_per_turn`；(4) 双方盲审均提交后，PairFlow 检查 `new_issues`：均空 → `blind_review_pending=false` → 可进 checklist；任一方非空 → `blind_review_pending=false`，进入交替评审处理新 issue；(5) **提出者不修改强制校验**：正式阶段 PairFlow 校验 `resolved_issue_ids` 中是否包含 `raised_by = 当前持笔者` 的 issue → 若包含则拒绝（"issue #N was raised by you; the other party must land the spec change"）。bootstrap 阶段靠对方审查 + §17 教训记录约束 |
