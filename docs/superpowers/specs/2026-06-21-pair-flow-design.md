...
监督者
  ...
  │
  │  confirm_task({ task_path:"/absolute/path/docs/task/xxx.md" })
  ├───────────────────────────────►
  │  PairFlow: 校验 task_path 在 work_dir 下；
  │            检查 {task}.pid → 存在则从 handoff 恢复状态
...
**身份判定**：
- HTTP header `X-AI-Identity: <token 或 自报名称>`
- 无有效 header → `"unknown"`，仅 `ping` / `who_am_i` 可用
- `register` 返回 UUID token，后续请求用 token 值替代明文身份名放入 header
- 服务端维护进程内 `token → identity` 映射，`parseIdentity` 自动将 token 解析为对应身份；明文 identity 原样返回（向后兼容）
...
**崩溃恢复**：启动时清 `.pairflow/`，每次全新开始。崩溃恢复由用户主动触发——`confirm_task` 发现任务文档已有 `.pid` 文件时读取 workflow_id 并从 `handoff/` 恢复状态（phase、round、peers、last_submit_per_turn 等）。恢复不自动扫描——`confirm_task` 是唯一恢复入口。
...
| `register` | `{ identity: string, supervisor: bool, developer: bool, work_dir: string }` | IDLE 阶段注册。identity 从 body 取（不再从 header）。仅 IDLE 下可用。identity 已存在 → 拒绝。supervisor/developer 各唯一但可兼任。work_dir 必填，双方校验一致性。返回 UUID `token`，后续请求用 token 替代明文 identity 放入 `X-AI-Identity` header。入参校验失败（identity/work_dir 缺失、identity 非法字符）返回 curl 格式参考 + 尖括号变量说明。`tip` 包含身份标签 + role 标注 + **结对边界声明**（结对编程由两个独立 AI 各自在独立 session 中注册，不得在同一 session 注册双方身份）；监督者 tip 显式注明 `confirm_dir` 的 `work_dir` 参数 |
| `confirm_dir` | `{ work_dir: string }` | `{ work_dir, incomplete_workflows[] }` | 确认工作目录，返回未完成的工作流列表（含 task_path）。仅监督者在 IDLE 阶段可用。未完成判断：扫 `handoff/` 目录 → `handoff/{id}/summary/` 有文件 → 已完成；无 summary 目录 + `{task}.pid` 存在 → 未完成。`tip` 使用 `[行动]`/`[当前]` 两层格式。无未完成→提示询问用户输入任务文档路径；有→列出 A/B 选项 |
| `confirm_task` | `{ task_path: string }` | `{ task_path, workflow_id, phase, recovered }` | 确认任务文档路径（必须为绝对路径，且在 `work_dir` 下）。**校验任务文件真实存在**，不存在则拒绝。在任务文档同目录创建 `{文档名}.pid` 文件存储 workflow_id。若 `.pid` 已存在 → 从 handoff 恢复流程状态，返回 `recovered:true`；否则全新开始。仅监督者在 IDLE 阶段可用。`tip` 按新建/恢复分支，均包含身份标签；指引 AI 先向用户报告状态再操作；恢复时按 `turn===identity` 区分 claim_turn vs wait_for_turn |
| `advance` | 无 | `{ ok, new_phase, turn, sub_phase? }` | 推进到下一阶段。仅监督者可用，**且需 `turn === "idle" \|\| turn === identity`**——监督者只能在自己 turn 时推进，防止跳过对方审阅轮。各 phase 转换规则同上。SUMMARY→IDLE 时要求至少一次 summary submit，删除 `.pid` 文件，清空 peers。`tip` 使用 `[行动]`/`[产出]`/`[当前]` 三层格式，turn 归属用自然语言（`turnIsSelf` 判断，不再硬编码"对方"），IDLE 结束包含归档位置和重新开始指引 |
| `claim_turn` | 无 | `{ ok }` | 获取当前轮次的执行权。仅在自己 turn 时可用。记录 `turn_claimed_at`。`tip` 使用 `[行动]`/`[产出]`/`[当前]` 三层自然语言格式（由 `buildTip()` 生成），按 phase+round 动态组合行动指令 |
| `wait_for_turn` | 无 | `{ turn, phase, round, warning? }` | 长轮询等待 turn 切换。10s 间隔，600s 超时。`tip` 使用 `[行动]`/`[当前]` 两层格式（无产出文件）。**超时(600s)或掉线(30min)时不再建议继续轮询，改为建议向用户报告当前状态** |
...
`tip.ts` 导出 `buildTip(state, identity)`，被 `claim_turn` 和 `get_state` 复用。内部拆分为三个辅助函数：
