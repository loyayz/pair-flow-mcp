...
| `confirm_task` | `{ task_path: string, task_type?: "requirements" \| "development" }` | `{ task_path, workflow_id, phase, recovered }` | 确认任务文档路径（必须为绝对路径，且在 `work_dir` 下）。**校验任务文件真实存在**，不存在则拒绝。在任务文档同目录创建 `{文档名}.pid` 文件存储 workflow_id。若 `.pid` 已存在 → 从 handoff 恢复流程状态，返回 `recovered:true`；否则全新开始。`task_type` 可选，默认 `"development"`——需求模式跳过 planning/implementation。仅监督者在 IDLE 阶段可用。`tip` 按新建/恢复分支，均包含身份标签；指引 AI 先向用户报告状态再操作；恢复时按 `turn===identity` 区分 claim_turn vs wait_for_turn |
