...
> **已修复**（commit `3266f38`）：`findLatestWorkflowId` 改为按目录名时间戳降序取最新，不再使用评分算法。

### 4.4 全新开始的不可达性 —— 第三次重启验证

session 结束后重启服务尝试全新注册，发现了一条**无法从非 IDLE 状态到达 IDLE** 的路径：

**操作序列**：

```
1. 服务重启（最新代码）
2. claude 调用 register → "register only allowed in IDLE phase, current: planning"
3. get_state → planning, turn=deepseek, peers 已恢复（幽灵注册）
4. 停止服务 → rm -rf .pairflow → 重启  ← 试图清空状态
5. claude 调用 register → "current: planning"  ← 仍然失败！
6. 停止服务 → mv handoff .handoff-backup-2 → rm -rf .pairflow → 重启
7. claude 调用 register → 成功（真正干净的状态）
```

**暴露的三个问题**：

**问题 A：删 `.pairflow/` 不足以清空状态**。`recoverState()` 在 `loadState()` 返回 defaultState（phase=idle, workflow_id=null）时触发 `reconstructFromHandoff()`，从 handoff 目录重建完整状态。只要 handoff 存在未完成的 workflow，删除 .pairflow 毫无意义。

**问题 B：reset 工具在需要时不可用**。`reset`（Cycle 1 产出）只允许在 IDLE 阶段执行。但当状态被恢复到 planning 时，无法注册 → 无法 advance → 无法到达 IDLE → 无法 reset。这是一个死锁：
```
非 IDLE → 需要注册才能 advance → 注册需要 IDLE → 需要 reset 回 IDLE → reset 需要 IDLE
```

**问题 C：全新开始的唯一方式是把 handoff 移走**。这意味着丢失所有归档数据（两次 session 的分析、计划、盲审）。对于想保留归档但清理状态的需求，当前无解。

**根因**：crash recovery 是一个**无条件执行的自动过程**——它不判断"用户是否想要恢复"，只要 handoff 有未完成 workflow 就恢复。retro-1 §2.4 记录的"崩溃恢复不应总是自动执行"问题仍未解决。
...
| 17 | 全新开始入口——register 时允许选择"忽略恢复" | 检测到 recovered 状态时，register 返回 `recovered: true` 和可选参数 `fresh_start: true`，调用方明确选择从头开始。仅清空 .pairflow，不动 handoff |
...
5. **全新开始几乎不可能**——删 .pairflow 不够（handoff 触发恢复），reset 在非 IDLE 不可用，唯一方式是移走 handoff 归档。恢复机制缺少"不，谢谢，从头开始"的选择
