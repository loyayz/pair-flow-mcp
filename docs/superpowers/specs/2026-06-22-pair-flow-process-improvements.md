...
> **双方注册时均上报根目录，server 校验一致性**：
> 1. `register` 新增必填参数 `work_dir`：双方注册时均需传入项目根目录的绝对路径
> 2. 第一个注册方：server 记录其 `work_dir`，返回 `"work_dir": "/path/to/project"`
> 3. 第二个注册方：server 校验其 `work_dir` 与第一个是否一致。不一致 → 拒绝注册，返回 `"work_dir mismatch: expected /path/A, got /path/B"`
> 4. 双方 `work_dir` 一致后，handoff 写入 `{work_dir}/.pairflow-handoff/`
> 5. 不存在默认路径——双方必须显式传入，且必须一致
