...
> **监督者注册后，server 要求设置工作目录——不设默认值**：
> 1. 监督者 `register` 成功后，server 返回：`"work_dir": null, "hint": "set work_dir before advance"`
> 2. 新增 `set_work_dir` 工具（仅监督者，IDLE 阶段可用）：接收项目根目录绝对路径，handoff 写入 `{work_dir}/.pairflow-handoff/`
> 3. advance 前置检查：`work_dir` 为 null 时拒绝 advance，返回 `"set work_dir before advance"`
> 4. 工作目录一旦设定，同一 workflow 内不可更改
> 5. 不存在默认路径——必须由监督者显式设置
