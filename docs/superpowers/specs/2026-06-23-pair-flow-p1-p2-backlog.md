...
### P1: 「文档更新确认」语义不明确——self-check vs cross-check

**来源**: 本 session REQUIREMENTS 审阅

当前 template 中「## 文档更新确认」所有场景共用同一套措辞（"本次是否更新了相关文档"），读起来像自报。但其设计意图是交叉检查——审阅方确认产出方的改动是否需要更新文档。在 r1（产出方）场景下，没有"对方"可查，语义退化为 self-check。两个场景用同一措辞导致双方都按 self-check 理解，交叉检查机制失效。

**方案**: 区分两套措辞：
- **审阅方模板**：`"检查对方产出是否需要更新文档：<是/否> → 对方是否已更新：<是/否> → 如未更新，追问理由"`
- **产出方模板**：`"本次产出是否需要配套文档更新：<是/否> → 已更新的文档：<路径> → 如未更新，原因"`

**影响**：`src/template.ts` 中 REQUIREMENTS/PLANNING/IMPLEMENTATION(coding/fix/review)/SUMMARY 模板
...
### 本迭代已完成（2026-06-23，2 Cycles，commits 9d73558 + 3c3dc54）

| 项 | 内容 | Commit |
|----|------|--------|
| P1-17 | IMPLEMENTATION 文件命名含 sub_phase | 9d73558（确认已完成） |
| P1 | next 字段——接口返回值引导 AI 下一步 | 9d73558 |
| P1 | get_archived_file_content phase 参数 | 9d73558 |
| P0-26 | clean 脚本 + orphan 预警 | 9d73558 |
| P1 | 崩溃恢复 opt-out（recovered flag + reset 工具） | 3c3dc54 |
| P1 | 文档更新确认语义修复（cross-check vs self-check） | 3c3dc54 |
...
2. SSE 事件推送
3. rules_catalog 扩展
4. 回顾文档发现的改进项（见 2026-06-23-pair-flow-retrospective.md §四）
```
