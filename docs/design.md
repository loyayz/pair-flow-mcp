...
PairFlow 按 workflow 维护进程内变化版本和 waiter 集合。事件只表示“workflow 状态可能变化”，不携带 phase、turn、claim 或完成事实；waiter 每次被唤醒后都必须重新读取 Server state。状态变更、归档写入或终止操作失败时不得发布成功事件。
...
各 phase advance 时重置 `round=1`，为每个参与者初始化空的 `last_submission_by_participant[identity] = { round:null, sub_phase:null, commit_hash:null, submitted_at:null, file_path:null }`。新 phase 的 `turn_switched_at` 写入 advance 时间，`turn_claimed_at` 始终为 null，并为该 assigned turn 初始化新的提醒 generation；无论 turn 是否仍属于调用 advance 的监督者，都必须先经 `wait_for_turn → claim_turn` 取得新阶段完整指引。
