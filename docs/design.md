...
生产源码、构建产物和 package public/deep-import surface 均不提供 PairFlow 专用 MCP client transport、token 注入 helper 或其他 Agent client wrapper。Server 的集成测试直接使用上游 MCP SDK；仅测试目录内的局部 helper 可以减少重复，且不得进入生产构建或发布面。需要监听端口的测试使用动态独占端口，不依赖固定端口。
...
- `status:"in_progress"` 的 manifest 禁止包含 summary phase record；active SUMMARY 只保留此前 phase 的接受记录。SUMMARY 终态 advance 在内存中生成 summary acceptance 与 completion 字段，并通过一次原子写直接产生 `status:"completed"` manifest，不先写中间态 summary acceptance。读取到 `in_progress + phases.summary` 视为非法归档并安全失败，不提供旧实现中间态兼容恢复。
...
- 下一阶段 instruction 的 required references 由 manifest 中的已接受记录生成：planning 读取任务输入；implementation 的每一个 coding、review 和 convergence turn 都使用已接受 planning r1 的固定 `canonical_plan.file_path`，其 reference commit 使用该 planning record 的 `acceptance_commit`；summary 使用 archive root、已接受 requirements submission，并在 development workflow 中增加已接受 plan、最新 coding 和最新 review。被跳过或不存在的引用省略，客户端不得扫描目录自行改选权威输入。
- 合法 `in_progress` manifest 的已接受 records 是 phase 接受事实的自包含权威，不要求对应 `.meta.json` 存在，也不与可能存在的 `.meta.json` 比较；历史 meta 的缺失、损坏或冲突不能撤销已经接受的 phase。每个已接受 submission reference 指向的 `.md` 仍是后续阶段的必要输入，必须逐项确认位于该 workflow/phase 的无链接归档边界内，且为路径、文件名语义与 reference 一致的非空直属普通文件；缺失、损坏或矛盾时安全失败，不能静默改选另一个“看起来最新”的文件。恢复到 manifest 确定的 active phase 后，仅该尚未接受 phase 的 submissions 继续按 `.meta.json + .md` 生成契约重建；已接受 phase 的 meta 不参与恢复判定。manifest 缺失只可能表示第一次 phase acceptance 尚未发生，因此只能从 requirements 目录的合法 `.meta.json + .md` 恢复 active REQUIREMENTS；若 planning、implementation 或 summary 中存在符合 submission 生成契约的记录，则归档与“尚未发生首次 acceptance”矛盾，必须安全失败并保留 `.pid`，不能猜测 phase 或忽略该记录；所有 phase 均无合法 submission 时才按空归档规则创建新 Workflow。启动时处理 `completed` manifest 只严格验证 manifest 自身的路径/上下文/schema 一致性及顶层 `final_summary === phases.summary.final_summary`；completed manifest 已自包含最终交付引用，外部 `.meta.json` 是不进入 Git 的 MCP sidecar，因此任何历史 `.meta.json` 或 `.md`（包括 final summary 对应文件）的缺失/损坏都不得成为清理 stale `.pid` 或创建新 Workflow 的门禁，也不得撤销已经线性化的 Workflow Completion；实现可以写非阻塞日志告警，但不增加成功响应契约或要求用户修复 sidecar。

终态 SUMMARY advance 的顺序是：

1. 从当前合法 summary submissions 与既有 phase acceptance 在内存中生成完整的 `status:"completed"` manifest，并只原子写入一次；不先持久化 `in_progress` summary record，该 completed rename 是逻辑完成线性化点。
...
MCP Streamable HTTP 的 POST 请求头仍按上游协议同时声明 `Accept: application/json, text/event-stream`；这是客户端请求协商要求，不表示 PairFlow 可以选择 SSE 响应。`enableJsonResponse:true` 与响应 `Content-Type: application/json` 才是 Server 返回模式的权威。内置 Skill、curl 示例、cold-start raw client 和上游 MCP SDK Client 都使用双值 Accept，并分别验证实际响应只能是 JSON。
...
**崩溃恢复**：每次启动全新开始（状态全内存）。崩溃恢复由用户主动触发——`confirm_task` 发现任务文档已有 `.pid` 普通文件时读取 workflow_id。它先读取 delivery manifest：manifest 自身无法解析、schema/路径/上下文非法时安全失败，保留 `.pid` 与旧 archive，不猜测状态或创建新 workflow；响应报告 manifest 路径与错误，用户检查后可以修复 manifest，或明确删除 stale `.pid` 以放弃旧 workflow。合法 `completed` manifest 通过内部一致性校验后删除 stale PID（ENOENT 成功；其他错误拒绝并同时报告 manifest path 与错误码），然后在同一 confirm_task 调用中创建新 workflow，绝不重建 SUMMARY；任何历史 `.meta.json` 或 `.md` 的损坏都不撤销完成或阻止该清理。合法 `in_progress` manifest 的已接受 records 是 phase 边界权威：requirements 后进入 planning（requirements task 则 summary）、planning 后进入 implementation、implementation 后进入 summary；已接受 records 不依赖或比较对应 `.meta.json`，但其每个 `.md` 引用都必须通过 §3.1 的路径、文件类型、非空与 reference 一致性检查，否则安全失败。下个 active phase 无 submission 时恢复 round=1、空 last_submission 并在双方角色确认后按 phase 分配 assigned turn；只有该尚未接受 phase 已存在 submissions 时，才从本次入参指定的 `<work_dir>/handoff/` 中符合 meta 生成契约的记录恢复到最后一次成功 submission 后的状态（phase、round、参与者身份、last_submission_by_participant），已接受 phase 的 meta 不参与恢复判定。manifest 缺失时只扫描 requirements 来恢复尚未接受的 REQUIREMENTS；若 planning、implementation 或 summary 中存在合法 submission，则安全失败并保留 `.pid`，所有 phase 都没有合法 submission 时才创建新 Workflow。handoff 根、workflow 和 phase 目录不得包含符号链接；有 manifest 时只扫描 active phase 目录、无 manifest 时只用 requirements 目录的直属普通 meta 重建状态，但仍检查后续 phase 是否存在矛盾的合法 submission；嵌套文件和 meta 链接不参与恢复，损坏或字段不完整的 meta 被忽略，等同于对应 round 缺失。恢复后的 `turn_switched_at` 取当前 phase 最高 round submission 的 submitted_at，`turn_claimed_at` 为 null；双方身份确认齐全后，turn 指向该最高 round 提交者的对方。warning/ack 不写入归档也不恢复：首位真实参与者重新确认后，roster 周期从本次确认时间开始；双方真实身份恢复完整后，当前 turn 保持 assigned，并从 roster 恢复就绪时间建立新的 30 分钟 turn 周期。work_dir 和 task.spec_file 始终分别以本次 `confirm_task` 的 work_dir、task_path 入参为准；归档 meta 中的 spec_file 只作为历史记录，不参与当前路由；过滤后的有效记录中，task_type 必须一致，requirements 任务中的 planning/implementation 记录无效。`.pid` 只保存 workflow_id，不保存或约束旧 work_dir；其内容格式非法或该 work_dir 下不存在所指归档时，均视为用户要创建新任务，创建新 workflow 并覆盖 `.pid`。恢复只接受文件名中符合 identity 规则（小写字母、数字、下划线、连字符）的 canonical lowercase 记录；含大写字母或其他非法 identity 的文件名直接忽略，不兼容旧格式，过滤后合法 identity 超过两个则归档不可恢复；每个 phase 内有效 round 必须唯一且身份须符合奇偶轮交替，但允许历史 round 缺失，恢复后的 round 为当前 phase 现存最大 round + 1；IMPLEMENTATION 有效文件名必须包含 sub_phase，且奇数 round 为 coding、偶数 round 为 review。`.pid` 原子写入；过滤后无有效记录，或有效记录之间出现结构冲突时，归档不足以恢复，`confirm_task` 创建新 workflow 并覆盖 `.pid`，旧归档保持不变；`.pid` 或归档路径存在但类型错误、包含链接或无法读取时则拒绝恢复，并在 tip 中说明路径和文件系统错误，不覆盖 `.pid`；`.pid` 指向的 workflow_id 若已被另一活跃任务占用，同样拒绝，绝不覆盖活跃内存状态。summary 目录或 summary 文件存在不代表 workflow 已完成。参与者的职责（is_supervisor/is_developer）以恢复占位 Participant 首次重新确认时的 `confirm_task` 入参为准，不从归档推断；恢复占位参与者允许临时职责不完整，待双方都重新确认后必须重新满足职责组合规则。恢复仅识别出一个 identity 时，该身份确认后仍须等待第二位参与者加入；双方未完整就位前 `advance` / `submit` 仍拒绝执行，`wait_for_turn` 则保持等待直到 roster 完整，`get_state` 返回相同等待指引。恢复不自动扫描——`confirm_task` 是唯一恢复入口。

前段“过滤后无有效记录或存在结构冲突则创建新 Workflow”的空/冲突归档规则仅适用于 manifest 缺失：所有 phase 均无合法 submission 时可以新建；只有 requirements 存在 records 但其内部结构冲突而不足以恢复时也可以新建；后续 phase 存在合法 submission 时必须安全失败。合法 `in_progress` manifest 已给出接受边界，active phase 无合法 submission 时按 round=1 恢复，active phase 的合法 records 发生结构冲突时安全失败，不得覆盖 `.pid`。
...
运行时严格、公开投影兼容是两层独立要求。Server 运行时 Zod schema 必须拒绝跨场景字段混用：`who_am_i` 匿名、已注册未绑定和已加入 workflow 的字段组合各自闭合；`advance.sub_phase` 仅在进入 implementation 时出现，completion 字段仅在最终完成时出现；`get_state` 未绑定、正常绑定和 unsupported state 分支分别满足其完整条件；`wait_for_turn` 普通/timeout/warning 分支携带 round，phase 只在 unsupported state 时可省略，completion 分支不携带 round，warning 只对应两类 stale reason；所有业务拒绝都不得混入成功字段。以上对象级条件不要求 `tools/list` 把公开 schema 改成非 object union。
...
内置 PairFlow Setup Skill 以结构化 `instruction` 而不是 tip 或“首次 wait 返回”判断初始化是否完成：`WAIT_TIMEOUT` 自动再次调用 `wait_for_turn`；两类 stale warning 先报告用户并等待选择；`TURN_ASSIGNED` 自动调用无参 `claim_turn`；只有 `claim_turn`（或已 claimed 场景的 wait）返回当前 claimed Turn 的完整 produce/advance/convergence instruction 后，初始化才成功并把控制权交回正常任务执行；`WORKFLOW_COMPLETED` 则停止。单独的 `phase:"idle" / turn:"idle"` 不是 Skill 成功或完成判据。
...
冷启动采集在任何文件系统写入前必须完成 runtime preflight：health 必须声明 `instruction_v1`、`structured_tool_output_v1`、`json_response_v1` 与 `delivery_manifest_v1`，且 register、confirm_task、wait_for_turn、claim_turn、get_state、advance、submit 必须把预期 input 字段列入 input schema 的 `properties` 与 `required`，并提供顶层 object outputSchema；`wait_for_turn` 与 `claim_turn` 的 required input 均为空。采集脚本的每次 MCP POST 请求必须发送 `Accept: application/json, text/event-stream` 以符合 Streamable HTTP 协商要求，但响应必须收到 `application/json` 并直接解析 JSON-RPC envelope；收到 `text/event-stream` 视为运行中 Server 不符合当前协议。缺少任一能力、schema 或 JSON transport 契约时，脚本必须在创建 `runs/` 或本次 run 前失败。
