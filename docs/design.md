...
**崩溃恢复**：每次启动全新开始（状态全内存）。崩溃恢复由用户主动触发——`confirm_task` 发现任务文档已有 `.pid` 文件时读取 workflow_id 并从 `handoff/` 恢复状态（phase、round、peers、last_submit_per_turn 等）。恢复不自动扫描——`confirm_task` 是唯一恢复入口。
