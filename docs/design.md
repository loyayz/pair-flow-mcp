...
| `register` | `{ identity: string }` | IDLE 阶段注册身份。identity 从 body 取。不声明角色（角色移至 confirm_task）。返回 UUID `token`。入参校验失败返回 curl 格式参考。`tip` 详细列出 confirm_task 的 5 个入参（task_path/task_type/supervisor/developer/work_dir）及含义 |
