...
**崩溃恢复**：每次启动全新开始（状态全内存）。崩溃恢复由用户主动触发——`confirm_task` 发现任务文档已有 `.pid` 文件时读取 workflow_id，并从 `handoff/` 中已有 `.meta.json` 恢复到最后一次成功 submission 后的状态（phase、round、参与者身份、last_submission_by_participant）。恢复只接受文件名中符合 identity 规则（字母、数字、下划线、连字符）的归档记录；非法 identity 文件名忽略。`.pid` 存在表示 workflow 未完成；summary 目录或 summary 文件存在不代表 workflow 已完成。参与者的角色（supervisor/developer）以 `confirm_task` 入参为准，不推断；恢复占位参与者允许临时角色不完整，待双方都重新确认后必须重新满足角色组合规则。只要仍存在恢复占位参与者，`advance` / `wait_for_turn` / `submit` 都拒绝执行，提示所有恢复出的参与者先调用 `confirm_task`。恢复不自动扫描——`confirm_task` 是唯一恢复入口。
