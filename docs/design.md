...
以上工具中，`register`、`confirm_task`、`advance`、`get_state`、`wait_for_turn`、`submit` 的带 `tip` 响应均新增可选 `instruction` 字段（`PairFlowInstruction` 对象）。`ping` 和无行动的正常 `who_am_i` 响应不包含 `instruction`。HTTP 层和 MCP 协议层错误不套用 `instruction`。
...
自 v1 起，所有带 `tip` 的 MCP 业务响应新增可选 `instruction` 字段。`instruction` 是机器可读的结构化契约，与 `tip` 自然语言指引并行——`instruction` 告诉客户端**做什么**，`tip` 告诉 AI **如何思考**。两者必须由同一个场景选择结果生成，禁止分别判定或从已渲染的 `tip` 反向解析。

#### 10.5.1 权威边界

| 通道 | 消费者 | 格式 | 稳定性 |
|------|--------|------|--------|
| `instruction` | 客户端/CLI/Skill 开发者 | 结构化 JSON，封闭枚举 | 模板改写不影响；字段和枚举值受契约保护 |
| `tip` | AI 参与者 | 自然语言 `[行动]/[产出]/[当前]` | 模板可自定义；不供客户端解析 |

新客户端必须优先读取 `instruction`，仅将 `tip` 作为 AI 的自然语言思考指引展示。不支持 `instruction` 的旧客户端忽略该字段，行为不变。

#### 10.5.2 TypeScript 契约
...
5. 客户端遇到未知 reason_code 时应安全失败并提示升级 PairFlow Server，不得回退解析 `tip`。
