...
- **PairFlow**：中立调度方。两边可见相同工具集，执行权限按职责和 turn 控制。状态变更持 async-mutex（`confirm_task` 按规范化 `task_path` 加锁；工作流运行期按 workflow 加锁）
...
`project/` = `work_dir` = git 仓库根目录。状态为进程内存变量（重启后丢失），无运行时状态文件。归档产出（协作过程权威记录，纳入版本管理）固定置于 `<work_dir>/handoff/`，不受 PairFlow server 启动目录影响。
...
**meta.json 生成**：`.meta.json` 由 `submit` 工具在每次提交成功后自动生成，写入 `submitted_at`、`commit_hash`、`sub_phase`、`task` 字段。AI 无需手动创建。`.meta.json` 必须使用同目录临时文件 + rename 原子写入；只有 `.meta.json` 写入成功后，`submit` 才能推进内存状态并返回成功。崩溃恢复时 `reconstructFromHandoff` 从 `.meta.json` 重建 `last_submission_by_participant` 等状态字段。
...
**崩溃恢复**：每次启动全新开始（状态全内存）。崩溃恢复由用户主动触发——`confirm_task` 发现任务文档已有 `.pid` 文件时读取 workflow_id，并从本次入参指定的 `<work_dir>/handoff/` 中已有 `.meta.json` 恢复到最后一次成功 submission 后的状态（phase、round、参与者身份、last_submission_by_participant）。work_dir 始终以本次 `confirm_task` 入参为准，`.pid` 只保存 workflow_id，不保存或约束旧 work_dir；若该 work_dir 下不存在 `.pid` 所指归档，视为用户要在新目录创建任务，创建新 workflow 并覆盖 `.pid`。恢复只接受文件名中符合 identity 规则（字母、数字、下划线、连字符）的归档记录；非法 identity 文件名忽略。`.pid` 原子写入；对应归档存在但不足以恢复时，同样允许 `confirm_task` 创建新的 workflow 并覆盖 `.pid`。summary 目录或 summary 文件存在不代表 workflow 已完成。参与者的职责（is_supervisor/is_developer）以 `confirm_task` 入参为准，不推断；恢复占位参与者允许临时职责不完整，待双方都重新确认后必须重新满足职责组合规则。只要仍存在恢复占位参与者，`advance` / `wait_for_turn` / `submit` 都拒绝执行，提示所有恢复出的参与者先调用 `confirm_task`。恢复不自动扫描——`confirm_task` 是唯一恢复入口。
...
| `get_state` | 无 | `{ tip }` | 返回当前执行指引。`tip` 使用 `[行动]`/`[产出]`/`[当前]` 三层自然语言格式（由 `buildTip()` 生成）；若 workflow 恢复未完成，则只提示所有恢复出的参与者先调用 `confirm_task`，不生成正常行动路径；若当前 token 未加入活跃 workflow（例如旧 workflow 已结束并清空 participants），提示重新 `confirm_task` |
| `submit` | `{ file_path, git_commit_hash }` | `{ ok, next_turn }` | 提交产出。校验 turn 持有者、`file_path` 必须是绝对路径且不得包含 `.` 或 `..` 路径段，并且必须是当前 workflow/phase/round/identity 对应且已存在的普通 handoff 文件；`git_commit_hash` 必须是 7–40 位十六进制 Git hash，且不同于当前 phase 最近一次提交的 commit_hash。PairFlow 不执行 Git 命令验证 commit 是否存在，hash 真实性由调用方负责。记录到 last_submission_by_participant（含 file_path），round+1 并切换 turn。自动在产出文件同目录原子生成 `.meta.json`（submitted_at、commit_hash、sub_phase、task），无需 AI 手动创建；若 `.meta.json` 写入失败，submit 返回拒绝且不推进内存状态。IMPLEMENTATION 阶段 submit 后 sub_phase 在 coding ↔ review 交替。`tip` 中的身份标签复用 `tip.ts` 的 `identityLabel()`——单一来源，不再重复推断职责 |
| `get_archived_files` | `{ phase?, workflow_id?, work_dir? }` | `{ files[] }` | 列出归档文件名，不读取文件内容，返回路径统一使用 POSIX 正斜杠。`phase` 可选过滤（requirements/planning/implementation/summary）；当前 token 已加入活跃 workflow 时可省略 workflow_id/work_dir，并从 Participant 状态推断。历史查询或匿名查询必须同时显式传入合法单路径段 `workflow_id` 和绝对 `work_dir`；work_dir 必须是已存在的目录且不得包含 `.` 或 `..` 路径段。有效 work_dir 下不存在对应 workflow 时返回空列表；若 token 未加入活跃 workflow 且未提供完整查询坐标，则返回空列表 |
