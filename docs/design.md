...
| `wait_for_turn` | 无 | `{ turn, phase, round, warning? }` | 仅在两位真实参与者完整就位后长轮询等待 turn 切换。10s 间隔，600s 为单次请求上限；600s 超时后提示 AI 继续调用 `wait_for_turn`，不打断用户，且不重置 workflow 级未领取计时。同一 workflow + identity 的并发等待采用 latest-wins，新调用取消旧调用并在结束时释放登记。客户端取消请求时立即终止本次等待，不返回业务 tip，也不改变 workflow 状态。turn 到达时自动记录 `turn_claimed_at`，返回 `[行动]/[产出]/[当前]` 三层完整指引。累计 30 分钟未领取时返回掉线 warning，并建议向用户报告当前状态 |
