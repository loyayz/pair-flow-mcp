...
启动 PairFlow Server，完成当前 AI 的 `register`、`confirm_task`，并领取首次分配的 turn。结构化 `instruction` 是 workflow control 的唯一权威；tip 只用于思考、内容和质量，不用于判断下一步或完成状态。
...
它会等待 roster、turn、提醒边界或 deadline。每次响应都读取 structured `instruction`：

- `reason_code: "WAIT_TIMEOUT"`：自动再次调用 `wait_for_turn`。
- `reason_code: "PARTICIPANT_CONFIRMATION_STALE"` 或 `"TURN_UNCLAIMED_STALE"`：向用户报告当前等待已过期，并暂停等待用户选择。
- `reason_code: "TURN_ASSIGNED"`：自动无参调用 `claim_turn`，再读取其返回的完整 instruction。
- `reason_code: "WORKFLOW_COMPLETED"`：停止自动执行。

只有 `claim_turn` 返回完整的 `produce_and_submit`、`advance` 或 `decide_convergence` instruction 时初始化才完成；随后将该 instruction 交回正常任务执行。不得以首次 wait 响应、tip 或单独的 `phase:"idle" / turn:"idle"` 判定初始化完成或 workflow 结束。
...
| `PARTICIPANT_CONFIRMATION_STALE` 或 `TURN_UNCLAIMED_STALE` | 向用户报告 PairFlow 返回的具体状态，暂停并等待用户选择 |
| `WORKFLOW_COMPLETED` | workflow 已结束，停止自动执行 |
