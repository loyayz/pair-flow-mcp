...
function identityLabel(state: PairFlowState, identity: string): string {
  const peer = state.peers.find((p) => p.identity === identity);
  if (!peer) return `当前身份: ${safe(identity)}`;
  const roleLabel = peer.role === "supervisor" ? "supervisor" : (peer.is_developer ? "developer" : "reviewer");
  return `当前身份: ${safe(identity)}(${roleLabel})`;
}
...
export function buildTip(state: PairFlowState, identity: string): string {
  ...
  // P0-3: 身份边界前缀 — 让 AI 明确知道自己是谁、turn 属于谁
  const idLabel = identityLabel(state, identity);
  const turnOwner = state.turn === identity ? "你" : "对方";
  const prefix = `${idLabel}。turn: ${safe(state.turn)}(${turnOwner})，阶段: ${phase}，轮次: ${state.round}。`;

  if (state.round === 1) {
    if (state.phase === "requirements") {
      return `${prefix}请先读取任务文档 ${taskPath}，理解需求后进行需求分析。所有观点需注明提出人。产出文件路径: ${outFile}。${submitParams}`;
    }
    if (state.phase === "planning") {
      return `${prefix}请先读取任务文档 ${taskPath}，根据需求分析产出实施计划，不需要拆分里程碑。所有观点需注明提出人。产出文件路径: ${outFile}。${submitParams}`;
    }
    if (state.phase === "implementation" && state.sub_phase === "coding") {
      return `${prefix}请根据实施计划进行代码实现。实现后先自行 code review，修改至自己认为没有问题，再产出文档。产出文件路径: ${outFile}。${submitParams}`;
    }
    if (state.phase === "summary") {
      return `${prefix}请产出一份阶段总结草稿（将由对方审阅后形成最终报告），包含本阶段的关键决策、遗留问题和后续建议。产出文件路径: ${outFile}。${submitParams}`;
    }
  ...
  if (state.phase === "requirements" || state.phase === "planning") {
    if (state.phase === "requirements") {
      return `${prefix}请基于当前任务文档 ${taskPath}，审阅 ${prevInfo}。所有观点需注明提出人。双方均同意的点，请直接修改任务文档 ${taskPath}；不同意的点，请在产出文件中标注原因和建议。产出文件路径: ${outFile}。${submitParams}`;
    }
    ...
    return `${prefix}请基于当前计划文档 ${planDoc}，审阅 ${prevInfo}。所有观点需注明提出人。双方均同意的点，请直接修改计划文档 ${planDoc}；不同意的点，请在产出文件中标注原因和建议。产出文件路径: ${outFile}。${submitParams}`;
  }
  ...
  if (state.phase === "implementation" && state.sub_phase === "review") {
    ...
    if (state.round > 2) {
      ...
      return `${prefix}请结合实施计划 ${planFile}、上一轮你的评审文档 ${myPrevReview}，审阅对方的代码产出 ${prevInfo}。检查是否按计划实现、上一轮问题是否已解决、代码正确性和风格。产出文件路径: ${outFile}。${submitParams}${advanceHint}`;
    }
    return `${prefix}请结合实施计划 ${planFile}，审阅对方的代码产出 ${prevInfo}。检查是否按计划实现、代码正确性和风格。产出文件路径: ${outFile}。${submitParams}${advanceHint}`;
  }

  if (state.phase === "implementation" && state.sub_phase === "coding") {
    return `${prefix}请根据上一轮的评审意见修改代码。修改后先自行 code review，确认问题已解决，再产出文档。产出文件路径: ${outFile}。${submitParams}`;
  }

  // P0-2 + P1-1: SUMMARY round ≥ 2 — r2 审阅草稿，r3+ 交替修订
  if (state.phase === "summary") {
    if (state.round === 2) {
      return `${prefix}请审阅监督者的汇总草稿 ${prevInfo}，提出修改意见或补充遗漏。产出文件路径: ${outFile}。${submitParams}`;
    }
    return `${prefix}请基于上一轮审阅意见修订汇总文档 ${prevInfo}。产出文件路径: ${outFile}。${submitParams}${advanceHint}`;
  }
