...
function getAction(state: PairFlowState, identity: string): string {
  ...
  // prevFile: 查 last_submit_per_turn 中对方记录的 file_path
  ...
  if (state.round === 1) {
    if (state.phase === "requirements") {
      return `读取任务文档 ${taskPath}，进行需求分析。所有观点需注明提出人。${submitParams}`;
    }
    if (state.phase === "planning") {
      return `读取任务文档 ${taskPath}，根据需求分析产出实施计划，不需要拆分里程碑。所有观点需注明提出人。${submitParams}`;
    }
    if (state.phase === "implementation" && state.sub_phase === "coding") {
      return `根据实施计划进行代码实现。实现后先自行 code review，修改至自己认为没有问题，再产出文档。${submitParams}`;
    }
    if (state.phase === "summary") {
      return `产出一份阶段总结草稿（将由对方审阅后形成最终报告），包含本阶段的关键决策、遗留问题和后续建议。${submitParams}`;
    }
    return `[错误] 未知的阶段/子阶段组合: phase=${state.phase}, sub_phase=${state.sub_phase}, round=1`;
  }

  if (state.phase === "requirements" || state.phase === "planning") {
    if (state.phase === "requirements") {
      return `基于任务文档 ${taskPath}，审阅 ${prevInfo}。所有观点需注明提出人。双方均同意的点直接修改任务文档；不同意的点在产出文件中标注原因和建议。${submitParams}`;
    }
    ...
    const planDoc = r1Submitter
      ...
      : "计划文档";
    return `基于计划文档 ${planDoc}，审阅 ${prevInfo}。所有观点需注明提出人。双方均同意的点直接修改计划文档；不同意的点在产出文件中标注原因和建议。${submitParams}`;
  }
  ...
  if (state.phase === "implementation" && state.sub_phase === "review") {
    ...
    if (state.round > 2) {
      ...
      return `结合实施计划 ${planFile}、上一轮你的评审文档 ${myPrevReview}，审阅对方的代码产出 ${prevInfo}。检查是否按计划实现、上一轮问题是否已解决、代码正确性和风格。${submitParams}${advanceHint}`;
    }
    return `结合实施计划 ${planFile}，审阅对方的代码产出 ${prevInfo}。检查是否按计划实现、代码正确性和风格。${submitParams}${advanceHint}`;
  }

  if (state.phase === "implementation" && state.sub_phase === "coding") {
    return `根据上一轮的评审意见修改代码。修改后先自行 code review，确认问题已解决，再产出文档。${submitParams}`;
  }

  if (state.phase === "summary") {
    if (state.round === 2) {
      return `审阅监督者的汇总草稿 ${prevInfo}，提出修改意见或补充遗漏。${submitParams}`;
    }
    return `基于上一轮审阅意见修订汇总文档 ${prevInfo}。${submitParams}${advanceHint}`;
  }

  return `[错误] 未知的阶段/子阶段组合: phase=${state.phase}, sub_phase=${state.sub_phase}, round=${state.round}`;
}
...
export function buildTip(state: PairFlowState, identity: string): string {
  ...
  const status = `${identityLabel(state, identity)} | turn: ${safe(state.turn)}(${turnOwner}) | ${safe(state.phase)} | round: ${state.round}`;
  return `[行动] ${action}\n\n[文件] ${file}\n\n[状态] ${status}`;
}
